...
function getRecordLargeSmall(b) {
  ...
  if (!large || !(large in SMALLS_BY_LARGE)) {
    ...
    for (const [lg, list] of Object.entries(SMALLS_BY_LARGE)) {
      ...
      if (hit) { large = lg; if (!small) small = hit; break; }
    }
  }
...
function matchesByFormRules(b, constraints = {}) {
  ...
  if (wantLarge || wantSmalls.length > 0) {
    ...
    const { large: recLarge } = getRecordLargeSmall(b);

    if (wantLarge && wantSmalls.length > 0) {
      const largeHitRaw = rawTxt.includes(valueToLabel(wantLarge)) || rawTxt.includes(wantLarge);
      const smallHitRaw = wantSmalls.some((s) => rawTxt.includes(String(s)) || blob.includes(norm(String(s))));
...
/* =================== 初期値 =================== */
const INITIAL_MODAL = { ...defaultRegion };
const INITIAL_SIDE  = { ...defaultSide };
...
function TransferSearch({ onBack }) {
  ...
  const listTopRef = useRef(null); // 🔝 리스트 상단 앵커
  ...
  // 요약/저장
  const [modalConstraints, setModalConstraints] = useState({ ...INITIAL_MODAL });
  const [sideFilters, setSideFilters] = useState({ ...INITIAL_SIDE });
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (activeReqRef.current !== my) return; // 🛡️ stale 응답 무시
        const rows = Array.isArray(data?.results) ? data.results : Array.isArray(data) ? data : [];
        setSearchResults(rows);
        setTimeout(() => { if (activeReqRef.current === my) setLoading(false); }, 0);
      } catch (e) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (payload?.source && payload.source !== "transfer") return;
  ...
  const runSearch = async (regionConstraints, sideFiltersArg) => {
    const my = ++activeReqRef.current; // 최신 요청 번호
    try {
      setLoading(true);
      ...
      const data = await res.json();
      if (activeReqRef.current !== my) return; // 🛡️ 이전 요청 응답 무시
      ...
      if (needDetail && raw.length) {
        ...
        if (activeReqRef.current !== my) return; // 🛡️
      }
      ...
      if (DEBUG) {
        console.groupCollapsed("[Transfer runSearch] after filters");
        console.log("serverCount :", raw.length);
        console.log("clientCount :", final.length);
  ...
  /* エリア 탭 클릭 */
  const handleAreaTabClick = async (label) => {
    const largeVal = label === "首都圏" ? "" : labelToValue(label);
    setSelectedTab(label);

    const nextRegion = largeVal ? { ...defaultRegion, large_area: largeVal } : { ...defaultRegion };
    setModalConstraints(nextRegion);
    setSideFilters({ ...defaultSide });

    setSearchResults([]);
    await runSearch(nextRegion, defaultSide);
  };

  /* モーダル 검색 — 地域 + （駅/徒歩/検索条件を絞り込む） */
  const handleModalSearch = async (fromModal) => {
    ...
    setSideFilters(mergedFilters);

    setIsModalOpen(false);

    setSearchResults([]);
    await runSearch(nextRegion, mergedFilters);
  };
  ...
  const addFav = (item) => {
    ...
    if (favorites.find((f) => f.id === merged.id && f.deal_type === "譲渡情報")) {
  ...
  return (
    <>
      ...
      <div className="container my-4 transfer-page">
        <h2 className="mb-3 transfer-title">譲渡情報一覧</h2>

        <div className="row g-3">
          {/* 좌측 패널 */}
          <aside ref={asideRef} className="col-lg-3" id="filter-aside">
            {/* 선택중 조건 */}
            <div className="as-panel shadow-sm mb-3 selected-panel">
              <div className="as-header">選択中の条件</div>
              <div className="as-body selected-body">
                <div className={`selected-summary ${summaryText ? "" : "text-muted"}`} title={summaryText || "—"}>
                  {summaryText || "—"}
                </div>
            ...
            <div className="area-tabs">
              {AREA_TABS.map((label) => (
                <button
                  ...
                  onClick={() => handleAreaTabClick(label)}
                  type="button"
                >
                  {label}
                </button>
          ...
          <main className="col-lg-9">
            ...
            <div ref={listTopRef} className="mb-2 d-flex justify-content-between align-items-center">
              <div className="text-muted small">
                エリア表示：{selectedTab} ／ {results.length}件
              </div>
            ...
            {loading ? (
              ...
              pageItems.map((row, i) => {
                ...
                const contractTerm = G(b, ["contract_period"], "—");
                const renewalFee = G(b, ["renewal_fee"], "—");
                const maintenance = G(b, ["maintenance"], "") || (G(b, ["maint_neg"], "") ? "相談" : "—");
...
function FilterPanel({ filters, setFilters, onSearch, buttonRef }) {
  ...
  return (
    <div className="as-panel shadow-sm mt-3">
      <div className="as-header">検索条件を絞り込む</div>
      <div className="as-body">
