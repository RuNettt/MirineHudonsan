// src/pages/FavoritesTransfer.js
...
function Pager({ page, totalPages, onChange }) {
  ...
  return (
    <div className="d-flex justify-content-center align-items-center gap-2 mt-3 flex-wrap" role="navigation" aria-label="譲渡情報ページャー">
      <button
        type="button"
...
export default function FavoritesTransfer() {
  ...
  useEffect(() => {
    ...
    setItems(saved.filter((f) => f.deal_type === "譲渡情報"));
  }, []);
  ...
  const filteredItems = useMemo(() => {
    let arr = [...mergedItems];
    if (hideContacted) arr = arr.filter((x) => !x._contacted);
    ...
    if (sortKey === "rentAsc") {
      ...
      const w = (x) => Number(x.station1_walk ?? x.station_minutes ?? x.walk_minutes ?? x.徒歩 ?? Infinity);
  ...
  const handleRemove = (id) => {
    const saved = JSON.parse(localStorage.getItem("favorites")) || [];
    const updated = saved.filter((x) => !(x.id === id && x.deal_type === "譲渡情報"));
    localStorage.setItem("favorites", JSON.stringify(updated));
    setItems(updated.filter((f) => f.deal_type === "譲渡情報"));
    alert("削除しました");
  };
    ...
    <>
      ...
      <div className="container my-4 fav-page">
        <div className="fav-title-wrap">
          <h2 className="mb-3 fav-title">お気に入り譲渡情報一覧</h2>
        </div>

        {/* 탭 전환 (店舗物件 / 譲渡情報) */}
        <div className="fav-tabs" role="tablist" aria-label="お気に入りの種類">
          <Link to="/favorites" className="fav-tab" role="tab" aria-selected="false">
            店舗物件
          </Link>
          <Link to="/favorites-transfer" className="fav-tab active" role="tab" aria-selected="true">
            譲渡情報
          </Link>
        </div>
        ...
        {filteredItems.length === 0 ? (
          ...
          <>
            {pageItems.map((b, i) => {
              ...
              // ====== 필드 매핑 ======
              const station = G(b, ["station1", "station", "nearest_station", "駅名"], "");
              const walk = G(b, ["station1_walk", "station_minutes", "walk_minutes", "徒歩"], "");
              const m2 = G(b, ["m2", "面積㎡"], "");
              const tsubo = G(b, ["tsubo", "面積坪"], "");
              const floor = G(b, ["floor_value1", "floor", "階数表示"], "");
